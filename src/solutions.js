...
function snake(value) {
  ...
  value = value.match(" ").join("-").toLowerCase();
  return value;
}
...
function parseDateString(value) {
	if(value==null){
		throw 'Input is invalid cannot be null';
	}
	else if(value.split("-").length!=3){
		throw 'Input is invalid must be date in string format YYYY-MM-DD';
	}
	else if(value.split("-")[0].length!=4){
		throw 'Year is invalid must be 4digits';
	}
	else if(value.split("-")[1].length!=2){
		throw 'Month is invalid must be 2digits';
	}
	else if(value.split("-")[2].length!=2){
		throw 'Day is invalid must be 2digits';
	}
	else{
		var datestr=value.split("-")[0]+"-"+value.split("-")[2]+"-"+value.split("-")[1];
